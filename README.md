...
| --- | --- |
| 무엇을 골라야 할지 모름 | 기본 설정 |
| 대각선·세로·곡선 대사 검출 | PP-OCRv6 Medium |
| 여러 언어의 문자 인식 | PP-OCRv6 Medium |
...
앱 데이터의 `models` 폴더에 호환되는 ONNX 파일을 넣으면 초기 설정과 이후 설정 화면에서 선택할 수 있습니다. 별도 설정 파일은 사용하지 않으며 다음 이름과 구성만 인식합니다.

- `detection`: 파일명에 `rtdetr` 또는 `yolo`가 포함된 RT-DETR-v2·YOLOv8/11 ONNX 모델
- `ocr/<모델 이름>`: 같은 폴더 안의 `*det*.onnx`, `*rec*.onnx`, `inference.yml`로 구성된 PP-OCR DB + CTC 모델
- `inpainting`: 파일명에 `lama` 또는 `aot`가 포함된 호환 ONNX 모델

Windows의 기본 위치는 `%LOCALAPPDATA%\vibecleaner\models`입니다. 모델을 추가한 뒤 설정 창을 다시 열면 목록을 새로 검색합니다. 검출 모델의 클래스는 `0=말풍선`, `1 이상=텍스트` 순서여야 합니다.
...
원문 언어를 일본어로 설정하세요. PP-OCRv6 Medium이 회전된 글자 영역을 그대로 검출해 인식합니다. 그래도 틀리면 인스펙터에서 원문을 직접 수정한 뒤 다시 번역할 수 있습니다.
