...
from ..base import OCREngineNotReadyError
...
class PPOCRv6Engine(OCREngine):
    ...
    def initialize(
        ...
        if self.model_option.paths:
            ...
            rec_id = (
                ...
                if self.model_option.id == SMALL_OCR_MODEL
                else ModelID.PPOCR_V6_REC_MEDIUM
            )
            ...
            config_file = [
                p for n, p in rec_paths.items() if n.endswith((".yml", ".yaml"))
            ]
            config_path = config_file[0] if config_file else None

        providers = get_providers(device)
        sess_opt = _make_ppocr_session_options(self.rec_threads)
        self.rec_sess = make_session(
            rec_model, sess_options=sess_opt, providers=providers
        )
        if len(self.rec_sess.get_inputs()) != 1 or not self.rec_sess.get_outputs():
            raise ValueError(
                "PP-OCR CTC Recognition ONNX 모델은 입력과 출력이 각각 1개여야 합니다."
            )
        ...
        if config_path:
            ...
            charset = (model_config.get("PostProcess") or {}).get(
                "character_dict"
            ) or []
            if charset:
    ...
    def _ensure_det_session(self) -> None:
        ...
        if self.model_option.paths:
            ...
            det_id = (
                ...
                if self.model_option.id == SMALL_OCR_MODEL
                else ModelID.PPOCR_V6_DET_MEDIUM
            )
        ...
        self.det_sess = make_session(
            det_path, sess_options=sess_opt, providers=providers
        )
        if len(self.det_sess.get_inputs()) != 1 or not self.det_sess.get_outputs():
            raise ValueError(
                "PP-OCR DB Detection ONNX 모델은 입력과 출력이 각각 1개여야 합니다."
            )

    def _rec_infer(self, crops: List[np.ndarray]) -> Tuple[List[str], List[float]]:
        if self.rec_sess is None or self.decoder is None:
            raise OCREngineNotReadyError("PP-OCR recognition model is not ready")
        ...
        for target_w, idxs in buckets.items():
            ...
            bytes_per_sample = max(
                1, self.rec_img_shape[0] * self.rec_img_shape[1] * target_w * 4
            )
            memory_batch = max(1, (128 * 1024 * 1024) // bytes_per_sample)
            batch_size = max(1, min(int(self.rec_batch_size), memory_batch))
            for start in range(0, len(idxs), batch_size):
                chunk = idxs[start : start + batch_size]
                batch = [
                    rec_resize_norm(crops[crop_index], self.rec_img_shape, max_ratio)[
                        None, ...
                    ]
                    for crop_index in chunk
                ]
                ...
                for oi, t, s in zip(chunk, dec_texts, dec_confs):
                    texts[oi] = t
                    confs[oi] = float(s)
        return texts, confs
    ...
    def process_image(
        ...
        if self.use_text_lines and any(getattr(blk, "lines", None) for blk in blk_list):
            for blk in blk_list:
                ...
                adaptive_indices = [
                    ...
                    if _needs_adaptive_retry(candidate)
                ]
                if (
                    bool(adaptive_binarization)
                    and adaptive_indices
                    and _is_japanese_language(self.source_language)
                ):
                    adaptive_crops = [
                        _crop_line(
                            img,
                    ...
                    adaptive_results = self._recognize_line_crops(
                        [adaptive_crops[index] for index in adaptive_indices]
                    )
                    results = list(raw_results)
                    for index, adaptive in zip(adaptive_indices, adaptive_results):
                        results[index] = _choose_japanese_candidate(
                            raw_results[index], adaptive
                        )
                elif bool(adaptive_binarization) and adaptive_indices:
                    adaptive_crops = [
                        _crop_line(
                            img,
                            line,
        ...
        adaptive_bin = (
            True if adaptive_binarization is None else bool(adaptive_binarization)
        )
        strength = (
            2.0
            if adaptive_binarization_strength is None
            else float(adaptive_binarization_strength)
        )
        crops = []
        for quad in boxes:
...
def _choose_japanese_candidate(
    ...
    return (
        adaptive
        if _japanese_candidate_score(adaptive) > _japanese_candidate_score(raw) + 0.04
        else raw
    )


def _needs_adaptive_retry(candidate: Tuple[str, float]) -> bool:
    text, confidence = candidate
    value = str(text or "").strip()
    return not value or float(confidence) < 0.55 or bool(re.search(r"(.)\1{5,}", value))
