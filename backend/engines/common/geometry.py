...
def inscribed_layout_box(
    image: np.ndarray,
    bubble_bbox: BBox,
    padding_ratio: float = 0.15,
    fallback_shrink: float = 0.2,
) -> Optional[BBox]:
    """Find the largest inscribed rectangle within a bubble using distance transform.

    Unlike simple uniform shrinking, this analyzes the actual bubble shape
    to find the best text placement area. Works for circles, clouds, and
    irregular bubble shapes.

    Args:
        image: Source image (RGB/BGR numpy array)
        bubble_bbox: Bubble bounding box (x1, y1, x2, y2)
        padding_ratio: Fraction of inscribed area to shrink for safety margin
        fallback_shrink: Uniform shrink ratio if distance transform fails

    Returns:
        Layout box (x1, y1, x2, y2) or None if calculation fails
    """
    try:
        import cv2
    except ImportError:
    ...
    try:
        ...
        if len(bubble_roi.shape) == 3:
            gray = cv2.cvtColor(bubble_roi, cv2.COLOR_RGB2GRAY)
        else:
            gray = bubble_roi

        # Create binary mask: foreground (bubble content) vs background
        # Use Otsu's thresholding to separate bubble from page background
        _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

        # Check if binary is all white or all black (uniform image)
        if np.all(binary == 255) or np.all(binary == 0):
            return None

        # Ensure bubble content is white (255), background is black (0)
        # If most pixels are white, invert
        if np.mean(binary) > 128:
            binary = cv2.bitwise_not(binary)

        # Morphological operations to fill holes and smooth
        ...
        if max_val < 5:  # Too small to be useful
            return None

        # Use the farthest point to define an inscribed circle
        cx, cy = max_loc
        radius = int(max_val * 0.75)  # Safety margin

        # Define inscribed rectangle (square for now, could be optimized)
        ins_x1 = max(0, cx - radius)
        ins_y1 = max(0, cy - radius)
        ins_x2 = min(width, cx + radius)
        ins_y2 = min(height, cy + radius)

        # Apply padding ratio
        ins_width = ins_x2 - ins_x1
        ins_height = ins_y2 - ins_y1
        pad_x = int(ins_width * padding_ratio)
        pad_y = int(ins_height * padding_ratio)

        layout_x1 = x1 + ins_x1 + pad_x
        layout_y1 = y1 + ins_y1 + pad_y
        layout_x2 = x2 + ins_x2 - pad_x
        layout_y2 = y2 + ins_y2 - pad_y

        # Validate: ensure layout box is within bubble and has reasonable size
        if layout_x2 - layout_x1 < 10 or layout_y2 - layout_y1 < 10:
            return None
