...
from modules.detection.utils.geometry import shrink_bbox, inscribed_layout_box
...
def draw_text(image: np.ndarray, blk_list: List[TextBlock], font_pth: str, colour: str = "#000", init_font_size: int = 40, min_font_size=10, outline: bool = True):
    ...
    for blk in blk_list:
        ...
        # Automatically adjust maximum search size based on bounding box
        box_limit = int(width * 0.9) if getattr(blk, "direction", "") == "vertical" else int(height * 0.9)
        effective_init_font_size = max(init_font_size, box_limit)

        translation, font_size = pil_word_wrap(image, tbbox_top_left, font_pth, translation, width, height,
                                               align=blk.alignment, spacing=blk.line_spacing, init_font_size=effective_init_font_size, min_font_size=min_font_size)
        font = font.font_variant(size=font_size)
...
def get_best_render_area(blk_list: List[TextBlock], img, inpainted_img=None):
    ...
    # Uses distance transform for bubble-shaped regions, falls back to shrink

    for blk in blk_list:
        if blk.text_class == 'text_bubble' and blk.bubble_xyxy is not None:

            # Try distance transform first for better shape-aware layout
            layout_box = inscribed_layout_box(img, tuple(map(int, blk.bubble_xyxy)))

            if layout_box is not None:
                blk.xyxy[:] = [layout_box[0], layout_box[1], layout_box[2], layout_box[3]]
            elif blk.source_lang_direction == 'vertical':
                # Fallback to uniform shrink for vertical text
...
def pyside_word_wrap(
    ...
    # Automatically adjust the maximum search limit based on the ROI (bounding box) size
    # so that text can scale up/down to fill the bubble.
    box_limit = int(roi_width * 0.9) if vertical else int(roi_height * 0.9)
    effective_max_size = max(init_font_size, box_limit)
    lo, hi = min_font_size, effective_max_size
    while lo <= hi:
        mid = (lo + hi) // 2
