...
def fill_poly(
    ...
    """
    Fills a polygon on an image.

    Args:
        image (np.ndarray): The canvas (image) on which to draw.
    ...
    for polygon in polygons:
        ...
        cv2.fillPoly(image, [reshaped_poly.astype(np.int32)], color)
...
def connected_components(image: np.ndarray, connectivity: int = 4) -> tuple:
    ...
    """
    return cv2.connectedComponents((image > 0).astype(np.uint8), connectivity=connectivity)
...
def otsu_threshold(array: np.ndarray) -> tuple[float, np.ndarray]:
    ...
    thresh_val, result = cv2.threshold(
