    ...
    if expected_checksum:
        ...
        if calculated_checksum == expected_checksum:
            ...
            try:
                os.remove(file_path)
                logger.error(
                    f"Model {algo}: {calculated_checksum}, expected {algo}: {expected_checksum}, wrong model deleted. Please restart {APP_NAME}."
                )
            except Exception:
                logger.error(
                    f"Model {algo}: {calculated_checksum}, expected {algo}: {expected_checksum}, please delete {file_path} and restart {APP_NAME}."
                )
            raise RuntimeError(
                f"Model {algo} mismatch for {file_path}: got {calculated_checksum}, expected {expected_checksum}. "
                f"Please delete the file and restart {APP_NAME} or re-download the model."
            )
